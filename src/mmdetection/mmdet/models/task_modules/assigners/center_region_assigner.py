...
class CenterRegionAssigner(BaseAssigner):
    ...
    def assign(
        ...
        **kwargs
        ...
        #   to match large gts
        prior_and_gt_core_overlaps = self.iou_calculator(priors, gt_core, mode="iof")
        # The center point of effective priors should be within the gt box
        is_prior_in_gt_core = is_prior_in_gt & (
            prior_and_gt_core_overlaps > self.min_pos_iof
        )  # shape (n, k)

        is_prior_in_gt_shadow = (
            self.iou_calculator(priors, gt_shadow, mode="iof") > self.min_pos_iof
        )
        ...
        if num_gts == 0 or num_priors == 0:
            ...
            assigned_gt_ids = is_prior_in_gt_core.new_zeros(
                (num_priors,), dtype=torch.long
            )
            ...
            assigned_gt_ids, pixels_in_gt_shadow = self.assign_one_hot_gt_indices(
                is_prior_in_gt_core, is_prior_in_gt_shadow, gt_priority=sort_idx
            )

        if gt_instances_ignore is not None and gt_instances_ignore.bboxes.numel() > 0:
            # No ground truth or boxes, return empty assignment
            gt_bboxes_ignore = gt_instances_ignore.bboxes
            gt_bboxes_ignore = scale_boxes(gt_bboxes_ignore, scale=self.ignore_gt_scale)
            is_prior_in_ignored_gts = is_located_in(prior_centers, gt_bboxes_ignore)
    ...
    def assign_one_hot_gt_indices(
        ...
        gt_priority: Optional[Tensor] = None,
    ) -> Tuple[Tensor, Tensor]:
        ...
        if gt_priority is None:
            gt_priority = torch.arange(num_gts, device=is_prior_in_gt_core.device)
        ...
        assigned_gt_inds = is_prior_in_gt_core.new_zeros(
        ...
        #  is saved
        pair_priority = is_prior_in_gt_core.new_full(
            (num_bboxes, num_gts), -1, dtype=torch.long
        )
        ...
        matched_bbox_gt_inds = torch.nonzero(is_prior_in_gt_core, as_tuple=False)[:, 1]
        ...
        shadowed_gt_inds = torch.cat(
            (shadowed_gt_inds, torch.nonzero(is_prior_in_gt_core, as_tuple=False)),
            dim=0,
        )
