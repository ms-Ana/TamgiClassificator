...
class SemiBaseDetector(BaseDetector):
    ...
    def loss_by_pseudo_instances(
        ...
        unsup_weight = (
            self.semi_train_cfg.get("unsup_weight", 1.0)
            if pseudo_instances_num > 0
            else 0.0
        )
    ...
    def project_pseudo_instances(
        self, batch_pseudo_instances: SampleList, batch_data_samples: SampleList
    ) -> SampleList:
        """Project pseudo instances."""
        for pseudo_instances, data_samples in zip(
            batch_pseudo_instances, batch_data_samples
        ):
            data_samples.gt_instances = copy.deepcopy(pseudo_instances.gt_instances)
            data_samples.gt_instances.bboxes = bbox_project(
                data_samples.gt_instances.bboxes,
