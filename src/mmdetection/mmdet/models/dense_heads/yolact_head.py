...
class YOLACTHead(AnchorHead):
    ...
    def __init__(
        ...
        loss_bbox: ConfigType = dict(type="SmoothL1Loss", beta=1.0, loss_weight=1.5),
        ...
        init_cfg: OptMultiConfig = dict(
            type="Xavier", distribution="uniform", bias=0, layer="Conv2d"
        ),
        **kwargs
    ) -> None:
    ...
    def _init_layers(self) -> None:
        ...
        self.conv_coeff = nn.Conv2d(
            self.feat_channels, self.num_base_priors * self.num_protos, 3, padding=1
        )
    ...
    def loss_by_feat(
        ...
        if self.use_ohem:
            ...
            # check NaN and Inf
            assert (
                torch.isfinite(all_cls_scores).all().item()
            ), "classification scores become infinite or NaN!"
            assert (
                torch.isfinite(all_bbox_preds).all().item()
    ...
    def get_positive_infos(self) -> InstanceList:
        ...
        for coeff_pred_per_level in self._raw_positive_infos["coeff_preds"]:
            coeff_pred_per_level = coeff_pred_per_level.permute(0, 2, 3, 1).reshape(
                num_imgs, -1, self.num_protos
            )
    ...
    def _predict_by_feat_single(
        ...
        mlvl_scores = []
        mlvl_coeffs = []
        for cls_score, bbox_pred, coeff_pred, priors in zip(
            cls_score_list, bbox_pred_list, coeff_preds_list, mlvl_priors
        ):
        ...
        multi_bboxes = self.bbox_coder.decode(priors, bbox_pred, max_shape=img_shape)
    ...
    def _bbox_post_process(
        ...
        **kwargs
        ...
        if rescale:
            ...
            multi_bboxes /= multi_bboxes.new_tensor(img_meta["scale_factor"]).repeat(
                (1, 2)
            )
...
class YOLACTProtonet(BaseMaskHead):
    ...
    def __init__(
        ...
        init_cfg=dict(
            type="Xavier", distribution="uniform", override=dict(name="protonet")
        ),
    ) -> None:
        ...
        self.segm_branch = (
            SegmentationModule(num_classes=num_classes, in_channels=in_channels)
            if with_seg_branch
            else None
        )
    ...
    def _init_layers(self) -> None:
        ...
        for num_channels, kernel_size in zip(
            self.proto_channels, self.proto_kernel_sizes
        ):
            if kernel_size > 0:
                layer = nn.Conv2d(
                    in_channels, num_channels, kernel_size, padding=kernel_size // 2
                )
            else:
                if num_channels is None:
                    layer = InterpolateModule(
                        scale_factor=-kernel_size, mode="bilinear", align_corners=False
                    )
                else:
    ...
    def loss_by_feat(
        ...
        **kwargs
        ...
        # crop
        croped_mask_pred = self.crop_mask_preds(
            mask_preds, batch_img_metas, positive_infos
        )
        ...
        for idx in range(num_imgs):
            ...
            (
                ...
                croped_mask_pred[idx],
                segm_preds[idx],
                batch_gt_instances[idx],
                positive_infos[idx],
            )
            ...
            if self.with_seg_branch:
                if segm_targets is None:
                    loss = segm_preds[idx].sum() * 0.0
                else:
                    loss = self.loss_segm(
                        segm_preds[idx], segm_targets, avg_factor=segm_avg_factor
                    )
                loss_segm.append(loss)
            # mask loss
            total_pos += num_pos
            if num_pos == 0 or pos_mask_targets is None:
                loss = mask_preds.sum() * 0.0
            else:
                mask_preds = torch.clamp(mask_preds, 0, 1)
                loss = (
                    F.binary_cross_entropy(
                        mask_preds, pos_mask_targets, reduction="none"
                    )
                ...
                gt_bboxes_width = (
                    gt_bboxes_for_reweight[:, 2] - gt_bboxes_for_reweight[:, 0]
                ) / w
                gt_bboxes_height = (
                    gt_bboxes_for_reweight[:, 3] - gt_bboxes_for_reweight[:, 1]
                ) / h
            ...
            with torch.no_grad():
                ...
                for obj_idx in range(downsampled_masks.size(0)):
                    segm_targets[gt_labels[obj_idx] - 1] = torch.max(
                        segm_targets[gt_labels[obj_idx] - 1], downsampled_masks[obj_idx]
                    )
        else:
            segm_targets = None
    ...
    def crop_mask_preds(
        ...
        """
        croped_mask_preds = []
        for img_meta, mask_preds, cur_info in zip(
            batch_img_metas, mask_preds, positive_infos
        ):
    ...
    def crop_single(self, masks: Tensor, boxes: Tensor, padding: int = 1) -> Tensor:
    ...
    def sanitize_coordinates(
        self, x1: Tensor, x2: Tensor, img_size: int, padding: int = 0, cast: bool = True
    ) -> tuple:
    ...
    def predict_by_feat(
        ...
        **kwargs
        ...
        croped_mask_pred = self.crop_mask_preds(
            mask_preds, batch_img_metas, results_list
        )
...
class SegmentationModule(BaseModule):
    ...
    def __init__(
        ...
        init_cfg: ConfigType = dict(
            type="Xavier", distribution="uniform", override=dict(name="segm_conv")
        ),
    ) -> None:
        super().__init__(init_cfg=init_cfg)
        self.in_channels = in_channels
