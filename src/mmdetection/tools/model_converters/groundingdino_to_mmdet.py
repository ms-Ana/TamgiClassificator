...
def convert(ckpt):
    ...
    for k, v in list(ckpt.items()):
        ...
        if "module.bbox_embed" in k:
            ...
            k = k.replace("module.bbox_embed", "module.transformer.decoder.bbox_embed")
        ...
        if "module.backbone.0" in k:
            ...
            if "module.transformer.enc_output" in k and "norm" not in k:
                new_k = k.replace("module.transformer.enc_output", "memory_trans_fc")
            ...
            new_k = k.replace("module.transformer.tgt_embed", "query_embedding")
            ...
            if "ca_text" in new_k:
                ...
                if "in_proj_weight" in new_k:
                    new_k = new_k.replace("in_proj_weight", "attn.in_proj_weight")
...
def main():
    parser = argparse.ArgumentParser(description="Convert keys to mmdet style.")
    parser.add_argument(
        "src", default="groundingdino_swint_ogc.pth.pth", help="src model path or url"
    )
    # The dst path must be a full path of the new checkpoint.
    parser.add_argument(
        "dst", default="groundingdino_swint_ogc.pth_mmdet.pth", help="save path"
    )
