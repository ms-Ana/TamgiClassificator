...
class CenterPrior(nn.Module):
    ...
    def forward(
        self,
        anchor_points_list: List[Tensor],
        gt_instances: InstanceData,
        inside_gt_bbox_mask: Tensor,
    ) -> Tuple[Tensor, Tensor]:
        ...
        for slvl_points, stride in zip(anchor_points_list, self.strides):
            ...
            distance = (
                (single_level_points - gt_center) / float(stride) - instance_center
            ) ** 2
        ...
        if self.force_topk:
            gt_inds_no_points_inside = torch.nonzero(
                inside_gt_bbox_mask.sum(0) == 0
            ).reshape(-1)
            if gt_inds_no_points_inside.numel():
                topk_center_index = center_prior_weights[
                    :, gt_inds_no_points_inside
                ].topk(self.topk, dim=0)[1]
                temp_mask = inside_gt_bbox_mask[:, gt_inds_no_points_inside]
                inside_gt_bbox_mask[:, gt_inds_no_points_inside] = torch.scatter(
                    temp_mask,
                    dim=0,
...
class AutoAssignHead(FCOSHead):
    ...
    def get_pos_loss_single(
        ...
        p_pos_weight = (confidence_weight * center_prior_weights) / (
            (confidence_weight * center_prior_weights).sum(0, keepdim=True)
        ).clamp(min=EPS)
        reweighted_p_pos = (p_pos * p_pos_weight).sum(0)
        pos_loss = F.binary_cross_entropy(
            reweighted_p_pos, torch.ones_like(reweighted_p_pos), reduction="none"
        )
    ...
    def loss_by_feat(
        ...
        for gt_instances, inside_gt_bbox_mask in zip(
            batch_gt_instances, inside_gt_bbox_mask_list
        ):
            center_prior_weight, inside_gt_bbox_mask = self.center_prior(
                all_level_points, gt_instances, inside_gt_bbox_mask
            )
        ...
        for bbox_pred, encoded_targets, inside_gt_bbox_mask in zip(
            ...
            expand_mlvl_points = (
                mlvl_points[:, None, :]
                .expand(num_points, temp_num_gt, 2)
                .reshape(-1, 2)
            )
            encoded_targets = encoded_targets.reshape(-1, 4)
            expand_bbox_pred = (
                bbox_pred[:, None, :].expand(num_points, temp_num_gt, 4).reshape(-1, 4)
            )
        ...
        if all_num_gt == 0:
            ...
            dummy_center_prior_loss = (
                self.center_prior.mean.sum() * 0 + self.center_prior.sigma.sum() * 0
            )
    ...
    def _get_targets_single(
        ...
        if num_gts:
            ...
            inside_gt_bbox_mask = bbox_targets.new_zeros(
                (num_points, num_gts), dtype=torch.bool
            )
