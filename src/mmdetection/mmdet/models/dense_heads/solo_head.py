...
class SOLOHead(BaseMaskHead):
    ...
    def __init__(
        ...
        scale_ranges: tuple = ((8, 32), (16, 64), (32, 128), (64, 256), (128, 512)),
        pos_scale: float = 0.2,
        ...
        init_cfg: MultiConfig = [
            ...
            dict(
                type="Normal", std=0.01, bias_prob=0.01, override=dict(name="conv_cls")
            ),
        ],
    ) -> None:
    ...
    def resize_feats(self, x: Tuple[Tensor]) -> List[Tensor]:
        ...
        for i in range(len(x)):
            if i == 0:
                out.append(F.interpolate(x[0], scale_factor=0.5, mode="bilinear"))
            elif i == len(x) - 1:
                out.append(
                    F.interpolate(x[i], size=x[i - 1].shape[-2:], mode="bilinear")
                )
            else:
                out.append(x[i])
        return out
    ...
    def loss_by_feat(
        ...
        **kwargs
        ...
        pos_mask_targets, labels, pos_masks = multi_apply(
            self._get_targets_single, batch_gt_instances, featmap_sizes=featmap_sizes
        )
    ...
    def _get_targets_single(
        self, gt_instances: InstanceData, featmap_sizes: Optional[list] = None
    ) -> tuple:
        ...
        gt_masks = gt_instances.masks.to_tensor(dtype=torch.bool, device=device)

        mlvl_pos_mask_targets = []
        mlvl_labels = []
        mlvl_pos_masks = []
        for (lower_bound, upper_bound), stride, featmap_size, num_grid in zip(
            self.scale_ranges, self.strides, featmap_sizes, self.num_grids
        ):
            ...
            labels = (
                torch.zeros([num_grid, num_grid], dtype=torch.int64, device=device)
                + self.num_classes
            )
            ...
            pos_w_ranges = (
                0.5 * (hit_gt_bboxes[:, 2] - hit_gt_bboxes[:, 0]) * self.pos_scale
            )
            pos_h_ranges = (
                0.5 * (hit_gt_bboxes[:, 3] - hit_gt_bboxes[:, 1]) * self.pos_scale
            )
            ...
            for gt_mask, gt_label, pos_h_range, pos_w_range, valid_mask_flag in zip(
                hit_gt_masks,
                hit_gt_labels,
                pos_h_ranges,
                pos_w_ranges,
                ...
                for i in range(top, down + 1):
                    for j in range(left, right + 1):
                        ...
                        mask_target[index, : gt_mask.shape[0], : gt_mask.shape[1]] = (
                            gt_mask
                        )
                        pos_mask[index] = True
    ...
    def _predict_by_feat_single(
        ...
        mask_preds = F.interpolate(
            mask_preds.unsqueeze(0), size=upsampled_size, mode="bilinear"
        )[:, :, :h, :w]
        mask_preds = F.interpolate(
            mask_preds, size=img_meta["ori_shape"][:2], mode="bilinear"
        ).squeeze(0)
        masks = mask_preds > cfg.mask_thr
...
class DecoupledSOLOHead(SOLOHead):
        ...
        for i in range(self.num_levels):
            ...
            if not self.training:
                ...
                mask_pred_x = F.interpolate(
                    mask_pred_x.sigmoid(), size=upsampled_size, mode="bilinear"
                )
                mask_pred_y = F.interpolate(
                    mask_pred_y.sigmoid(), size=upsampled_size, mode="bilinear"
                )
                cls_pred = cls_pred.sigmoid()
                # get local maximum
                local_max = F.max_pool2d(cls_pred, 2, stride=1, padding=1)
    ...
    def loss_by_feat(
        ...
        **kwargs
        ...
        for img_id in range(num_imgs):
            ...
            for lvl in range(num_levels):
                ...
                mlvl_pos_mask_preds_x[lvl].append(
                    mlvl_mask_preds_x[lvl][img_id, xy_pos_indexes[img_id][lvl][:, 1]]
                )
                mlvl_pos_mask_preds_y[lvl].append(
                    mlvl_mask_preds_y[lvl][img_id, xy_pos_indexes[img_id][lvl][:, 0]]
                )
                mlvl_labels[lvl].append(labels[img_id][lvl].flatten())
        ...
        # dice loss
        loss_mask = []
        for pred_x, pred_y, target in zip(
            mlvl_pos_mask_preds_x, mlvl_pos_mask_preds_y, mlvl_pos_mask_targets
        ):
    ...
    def _get_targets_single(
        ...
        mlvl_pos_mask_targets, mlvl_labels, mlvl_pos_masks = (
            super()._get_targets_single(gt_instances, featmap_sizes=featmap_sizes)
        )

        mlvl_xy_pos_indexes = [
            (item - self.num_classes).nonzero() for item in mlvl_labels
        ]
    ...
    def predict_by_feat(
        ...
        **kwargs
        ...
        for img_id in range(len(batch_img_metas)):
            cls_pred_list = [
                mlvl_cls_scores[i][img_id].view(-1, self.cls_out_channels).detach()
            ...
            results = self._predict_by_feat_single(
                cls_pred_list, mask_pred_list_x, mask_pred_list_y, img_meta=img_meta
            )
            results_list.append(results)
        return results_list
    ...
    def _predict_by_feat_single(
        ...
        for lvl in range(1, self.num_levels):
            lvl_start_index[lvl_interval[lvl - 1] : lvl_interval[lvl]] *= lvl_interval[
                lvl - 1
            ]
            mask_lvl_start_index[lvl_interval[lvl - 1] : lvl_interval[lvl]] *= seg_size[
                lvl - 1
            ]
...
class DecoupledSOLOLightHead(DecoupledSOLOHead):
    ...
    def _init_layers(self) -> None:
        ...
        for i in range(self.stacked_convs):
            if self.dcn_cfg is not None and i == self.stacked_convs - 1:
