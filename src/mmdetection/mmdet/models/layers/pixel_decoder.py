...
class PixelDecoder(BaseModule):
    ...
    def forward(
        self, feats: List[Tensor], batch_img_metas: List[dict]
    ) -> Tuple[Tensor, Tensor]:
        """
        Args:
...
class TransformerEncoderPixelDecoder(PixelDecoder):
    ...
    def __init__(
        ...
        positional_encoding: ConfigType = dict(num_feats=128, normalize=True),
        init_cfg: OptMultiConfig = None,
    ) -> None:
        super().__init__(
    ...
    def forward(
        ...
        memory = self.encoder(
            query=feat_last, query_pos=pos_embed, key_padding_mask=padding_mask
        )
        # (batch_size, num_queries, c) -> (batch_size, c, h, w)
        memory = memory.permute(0, 2, 1).view(bs, self.encoder_embed_dims, h, w)
