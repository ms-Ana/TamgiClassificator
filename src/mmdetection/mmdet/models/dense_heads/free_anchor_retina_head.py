...
class FreeAnchorRetinaHead(RetinaHead):
    ...
    def loss_by_feat(
        ...
        anchor_list, _ = self.get_anchors(
            featmap_sizes=featmap_sizes, batch_img_metas=batch_img_metas, device=device
        )
        ...
        negative_loss = self.negative_bag_loss(cls_probs, box_probs).sum() / max(
    ...
    def positive_loss_single(
        ...
        with torch.no_grad():
            if len(gt_bboxes) == 0:
                image_box_prob = torch.zeros(
                    flat_anchors.size(0), self.cls_out_channels
                ).type_as(bbox_pred)
            else:
                ...
                t1 = self.bbox_thr
                t2 = object_box_iou.max(dim=1, keepdim=True).values.clamp(
                    min=t1 + 1e-12
                )
                ...
                if indices.numel() == 0:
                    ...
                    image_box_prob = torch.sparse_coo_tensor(
                        indices.flip([0]),
                        nonzero_box_prob,
                        size=(flat_anchors.size(0), self.cls_out_channels),
                    ).to_dense()
                # end
        ...
        loss_bbox = self.loss_bbox(
            bbox_pred[matched], matched_object_targets, reduction_override="none"
        ).sum(-1)
        matched_box_prob = torch.exp(-loss_bbox)

        # positive_losses: {-log( Mean-max(P_{ij}^{cls} * P_{ij}^{loc}) )}
        num_pos = len(gt_bboxes)
        positive_loss = self.positive_bag_loss(matched_cls_prob, matched_box_prob)
