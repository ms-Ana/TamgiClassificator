...
class CascadeRoIHead(BaseRoIHead):
    ...
    def init_bbox_head(
        self, bbox_roi_extractor: MultiConfig, bbox_head: MultiConfig
    ) -> None:
        ...
        if self.train_cfg is not None:
            for idx, rcnn_train_cfg in enumerate(self.train_cfg):
                ...
                self.bbox_sampler.append(
                    TASK_UTILS.build(
                        rcnn_train_cfg.sampler, default_args=dict(context=self)
                    )
                )
    ...
    def bbox_loss(
        self, stage: int, x: Tuple[Tensor], sampling_results: List[SamplingResult]
    ) -> dict:
        ...
        for stage in range(self.num_stages):
            ...
            if self.with_mask:
                mask_results = self.mask_loss(
                    stage, x, sampling_results, batch_gt_instances
                )
        ...
        for stage in range(self.num_stages):
            ...
            if stage < self.num_stages - 1:
                ...
                if bbox_head.custom_activation:
                    cls_scores = [
                        bbox_head.loss_cls.get_activation(s) for s in cls_scores
                    ]
                refine_rois_list = []
                for i in range(len(batch_img_metas)):
                    if rois[i].shape[0] > 0:
