...
def weighted_boxes_fusion(
    ...
    if len(weights) != len(bboxes_list):
        print(
            "Warning: incorrect number of weights {}. Must be: "
            "{}. Set weights equal to 1.".format(len(weights), len(bboxes_list))
        )
        weights = np.ones(len(bboxes_list))
    ...
    for t in range(len(boxes)):
        ...
        if len(boxes[t]) != len(scores[t]):
            print(
                ...
                "length of scores array: {} != {}".format(len(boxes[t]), len(scores[t]))
            )
            exit()
        ...
        if len(boxes[t]) != len(labels[t]):
            print(
                "Error. Length of boxes arrays not equal to "
                "length of labels array: {} != {}".format(len(boxes[t]), len(labels[t]))
            )
            exit()
