...
class CentripetalHead(CornerHead):
        ...
        for _ in range(self.num_feat_levels):
            self.tl_feat_adaption.append(
                DeformConv2d(
                    self.in_channels,
                    self.in_channels,
                    self.feat_adaption_conv_kernel,
                    1,
                    1,
                )
            )
    ...
    def forward_single(self, x: Tensor, lvl_ind: int) -> List[Tensor]:
        ...
        """
        tl_heat, br_heat, _, _, tl_off, br_off, tl_pool, br_pool = (
            super().forward_single(x, lvl_ind, return_pool=True)
        )
    ...
    def loss_by_feat_single(
        ...
        tl_off: Tensor,
        br_off: Tensor,
        tl_guiding_shift: Tensor,
        br_guiding_shift: Tensor,
