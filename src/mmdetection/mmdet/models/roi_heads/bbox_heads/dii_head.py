...
class DIIHead(BBoxHead):
    ...
    def __init__(
        ...
        **kwargs
        ...
        assert self.reg_class_agnostic, (
            "DIIHead only " "suppport `reg_class_agnostic=True` "
        )
    ...
    def forward(self, roi_feat: Tensor, proposal_feat: Tensor) -> tuple:
        ...
        cls_score = self.fc_cls(cls_feat).view(
            N,
            num_proposals,
            self.num_classes if self.loss_cls.use_sigmoid else self.num_classes + 1,
        )
        ...
        return (
            cls_score,
            bbox_delta,
            obj_feat.view(N, num_proposals, self.in_channels),
            attn_feats,
        )
