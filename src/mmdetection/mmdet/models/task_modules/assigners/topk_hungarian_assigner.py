...
class TopkHungarianAssigner(TaskAlignedAssigner):
    ...
    def assign(
        ...
        **kwargs
        ...
        if num_gt > 0:
            # assign 0 by default
            assigned_gt_inds = pred_scores.new_full((num_bboxes,), 0, dtype=torch.long)
            ...
            repeat_select_cost = (
                select_cost[..., None].repeat(1, 1, topk).view(select_cost.size(0), -1)
            )
            # anchor index and gt index
            matched_row_inds, matched_col_inds = linear_sum_assignment(
                repeat_select_cost.detach().cpu().numpy()
            )
            matched_row_inds = torch.from_numpy(matched_row_inds).to(pred_scores.device)
            matched_col_inds = torch.from_numpy(matched_col_inds).to(pred_scores.device)
            ...
            overlaps = self.iou_calculator(
                decode_bboxes[candidate_idxs], gt_bboxes[match_gt_ids], is_aligned=True
            ).detach()

            temp_pos_alignment_metrics = alignment_metrics[candidate_idxs]
            pos_alignment_metrics = torch.gather(
                temp_pos_alignment_metrics, 1, match_gt_ids[:, None]
            ).view(-1)
            ...
            assigned_gt_inds = pred_scores.new_full((num_bboxes,), -1, dtype=torch.long)

            assigned_labels = pred_scores.new_full((num_bboxes,), -1, dtype=torch.long)

            assigned_gt_inds[:] = 0
            return AssignResult(0, assigned_gt_inds, None, labels=assigned_labels)
