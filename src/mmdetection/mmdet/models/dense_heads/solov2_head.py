...
class MaskFeatModule(BaseModule):
    ...
    def forward(self, x: Tuple[Tensor]) -> Tensor:
        ...
        for i in range(1, len(inputs)):
            input_p = inputs[i]
            if i == len(inputs) - 1:
                coord_feat = generate_coordinate(input_p.size(), input_p.device)
...
class SOLOV2Head(SOLOHead):
    ...
    def forward(self, x):
        ...
        for i in range(self.num_levels):
            ...
            coord_feat = generate_coordinate(
                ins_kernel_feat.size(), ins_kernel_feat.device
            )
    ...
    def _get_targets_single(
        ...
        mlvl_labels = []
        mlvl_pos_masks = []
        for (lower_bound, upper_bound), num_grid in zip(
            ...
            for gt_mask, gt_label, pos_h_range, pos_w_range, valid_mask_flag in zip(
                ...
                if not valid_mask_flag:
                    continue
                upsampled_size = (
                    featmap_sizes[0] * self.mask_stride,
                ...
                for i in range(top, down + 1):
                    for j in range(left, right + 1):
                        ...
                        this_mask_target[: gt_mask.shape[0], : gt_mask.shape[1]] = (
                            gt_mask
                        )
    ...
    def loss_by_feat(
        ...
        **kwargs,
            ...
            for img_id, img_lvl_pos_kernel_pred in enumerate(lvl_pos_kernel_preds):
                ...
                img_lvl_mask_pred = F.conv2d(
                    ...
                    img_lvl_pos_kernel_pred.permute(1, 0).view(
                        num_kernel, -1, self.dynamic_conv_size, self.dynamic_conv_size
                    ),
                    stride=1,
                ).view(-1, h, w)
        ...
        for lvl_mask_preds, lvl_mask_targets in zip(mlvl_mask_preds, mlvl_mask_targets):
            ...
            loss_mask.append(
                self.loss_mask(
                    lvl_mask_preds, lvl_mask_targets, reduction_override="none"
                )
            )
        if num_pos > 0:
        ...
        # cate
        flatten_labels = [
            torch.cat([img_lvl_labels.flatten() for img_lvl_labels in lvl_labels])
