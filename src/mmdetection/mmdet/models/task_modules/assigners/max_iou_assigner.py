...
def _perm_box(
    bboxes, iou_calculator, iou_thr=0.97, perm_range=0.01, counter=0, max_iter=5
):
    ...
    def assign(
        ...
        **kwargs
        ...
        if self.perm_repeat_gt_cfg is not None and priors.numel() > 0:
            gt_bboxes_unique = perm_repeat_bboxes(
                gt_bboxes, self.iou_calculator, self.perm_repeat_gt_cfg
            )
        else:
