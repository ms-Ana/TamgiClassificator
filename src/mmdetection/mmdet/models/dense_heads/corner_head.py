...
class CornerHead(BaseDenseHead):
    ...
    def __init__(
        ...
        loss_embedding: ConfigType = dict(
            type="AssociativeEmbeddingLoss", pull_weight=0.25, push_weight=0.25
        ),
        loss_offset: ConfigType = dict(type="SmoothL1Loss", beta=1.0, loss_weight=1),
        init_cfg: OptMultiConfig = None,
    ) -> None:
        ...
        self.loss_heatmap = (
            MODELS.build(loss_heatmap) if loss_heatmap is not None else None
        )
        self.loss_embedding = (
            MODELS.build(loss_embedding) if loss_embedding is not None else None
        )
        self.loss_offset = (
            MODELS.build(loss_offset) if loss_offset is not None else None
        )
    ...
    def _make_layers(
        self, out_channels: int, in_channels: int = 256, feat_channels: int = 256
    ) -> nn.Sequential:
    ...
    def _init_corner_kpt_layers(self) -> None:
        ...
        for _ in range(self.num_feat_levels):
            self.tl_pool.append(
                BiCornerPool(
                    self.in_channels, ["top", "left"], out_channels=self.in_channels
                )
            )
            self.br_pool.append(
                BiCornerPool(
                    self.in_channels, ["bottom", "right"], out_channels=self.in_channels
                )
            )
    ...
    def forward_single(
        self, x: Tensor, lvl_ind: int, return_pool: bool = False
    ) -> List[Tensor]:
    ...
    def get_targets(
        ...
        with_centripetal_shift: bool = False,
    ) -> dict:
        ...
        for batch_id in range(batch_size):
            ...
            for box_id in range(len(gt_labels[batch_id])):
                ...
                if with_guiding_shift:
                    gt_tl_guiding_shift[batch_id, 0, top_idx, left_idx] = (
                        scale_center_x - left_idx
                    )
                    gt_tl_guiding_shift[batch_id, 1, top_idx, left_idx] = (
                        scale_center_y - top_idx
                    )
    ...
    def loss_by_feat_single(
        ...
        tl_hmp: Tensor,
        br_hmp: Tensor,
        tl_emb: Optional[Tensor],
        br_emb: Optional[Tensor],
        tl_off: Tensor,
        br_off: Tensor,
        targets: dict,
    ) -> Tuple[Tensor, ...]:
    ...
    def predict_by_feat(
        ...
        """
        assert tl_heats[-1].shape[0] == br_heats[-1].shape[0] == len(batch_img_metas)
        result_list = []
    ...
    def _predict_by_feat_single(
        ...
        if rescale and "scale_factor" in img_meta:
            batch_bboxes /= batch_bboxes.new_tensor(img_meta["scale_factor"]).repeat(
                (1, 2)
            )
    ...
    def _bboxes_nms(
        self, bboxes: Tensor, labels: Tensor, cfg: ConfigDict
    ) -> Tuple[Tensor, Tensor]:
        """bboxes nms."""
        if "nms_cfg" in cfg:
            warning.warn(
        ...
        if labels.numel() > 0:
            ...
            bboxes, keep = batched_nms(
                bboxes[:, :4], bboxes[:, -1].contiguous(), labels, cfg.nms
            )
            if max_num > 0:
    ...
    def _decode_heatmap(
        ...
        if with_centripetal_shift:
            tl_centripetal_shift = (
                transpose_and_gather_feat(tl_centripetal_shift, tl_inds)
                .view(batch, k, 1, 2)
                .exp()
            )
            br_centripetal_shift = (
                transpose_and_gather_feat(br_centripetal_shift, br_inds)
                .view(batch, 1, k, 2)
                .exp()
            )
        ...
        if with_centripetal_shift:
            ...
            rcentral[..., 0] = (
                bboxes_center_x - mu * (bboxes[..., 2] - bboxes[..., 0]) / 2
            )
            rcentral[..., 1] = (
                bboxes_center_y - mu * (bboxes[..., 3] - bboxes[..., 1]) / 2
            )
            rcentral[..., 2] = (
                bboxes_center_x + mu * (bboxes[..., 2] - bboxes[..., 0]) / 2
            )
            ...
            area_rcentral = (
                (rcentral[..., 2] - rcentral[..., 0])
                * (rcentral[..., 3] - rcentral[..., 1])
            ).abs()
