...
def parse_args():
    ...
    parser.add_argument(
        "--save-fusion-results", action="store_true", help="whether save fusion result"
    )
    parser.add_argument(
        "--out-dir",
        type=str,
...
def main():
    ...
    assert len(args.models_name) == len(
        args.pred_results
    ...
    if args.save_fusion_results:
        ...
        print_log(f"Fusion results have been saved to {out_file}.", logger="current")
