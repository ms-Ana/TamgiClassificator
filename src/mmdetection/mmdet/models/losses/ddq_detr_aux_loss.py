...
class DDQAuxLoss(nn.Module):
    ...
    def __init__(
        ...
        train_cfg=dict(
            assigner=dict(type="TopkHungarianAssigner", topk=8), alpha=1, beta=6
        ),
    ):
    ...
    def loss(self, cls_scores, bbox_preds, gt_bboxes, gt_labels, img_metas, **kwargs):
        ...
        (labels_list, label_weights_list, bbox_targets_list, alignment_metrics_list) = (
            cls_reg_targets
        )
    ...
    def _get_target_single(
        self, cls_scores, bbox_preds, gt_bboxes, gt_labels, img_meta, **kwargs
    ):
        ...
        if len(gt_labels) == 0:
            ...
            labels = bbox_preds.new_full(
                (num_valid_anchors,), cls_scores.size(-1), dtype=torch.long
            )
        ...
        assign_result = self.assigner.assign(
            cls_scores, bbox_preds, gt_bboxes, gt_labels, img_meta
        )
