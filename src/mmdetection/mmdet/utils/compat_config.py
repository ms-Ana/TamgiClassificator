...
def compat_loader_args(cfg):
    ...
    if "samples_per_gpu" in cfg.data:
        ...
        assert "samples_per_gpu" not in cfg.data.train_dataloader, (
            "`samples_per_gpu` are set "
            "in `data` field and ` "
    ...
    if "persistent_workers" in cfg.data:
        ...
        assert "persistent_workers" not in cfg.data.train_dataloader, (
            "`persistent_workers` are set "
            "in `data` field and ` "
            "data.train_dataloader` "
            "at the same time. "
            "Please only set it in "
    ...
    if "samples_per_gpu" in cfg.data.val:
        ...
        assert "samples_per_gpu" not in cfg.data.val_dataloader, (
            "`samples_per_gpu` are set "
    ...
    if isinstance(cfg.data.test, dict):
        if "samples_per_gpu" in cfg.data.test:
            assert "samples_per_gpu" not in cfg.data.test_dataloader, (
                "`samples_per_gpu` are set "
            ...
            cfg.data.test_dataloader["samples_per_gpu"] = cfg.data.test.pop(
                "samples_per_gpu"
            )
