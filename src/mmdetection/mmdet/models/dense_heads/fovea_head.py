...
class FoveaHead(AnchorFreeHead):
    ...
    def loss_by_feat(
        ...
        if num_pos > 0:
            ...
            loss_bbox = self.loss_bbox(
                pos_bbox_preds, pos_bbox_targets, pos_weights, avg_factor=num_pos
            )
        else:
            loss_bbox = torch.tensor(
                0, dtype=flatten_bbox_preds.dtype, device=flatten_bbox_preds.device
            )
    ...
    def _get_targets_single(
        ...
        for base_len, (lower_bound, upper_bound), stride, featmap_size, priors in zip(
            self.base_edge_list,
            self.scale_ranges,
            self.strides,
            featmap_size_list,
            ...
            bbox_targets = gt_bboxes_raw.new_ones(featmap_size[0], featmap_size[1], 4)
            # scale assignment
            hit_indices = (
            ...
            pos_left = (
                torch.ceil(gt_bboxes[:, 0] + (1 - self.sigma) * half_w - 0.5)
                .long()
                .clamp(0, featmap_size[1] - 1)
            )
            pos_right = (
                torch.floor(gt_bboxes[:, 0] + (1 + self.sigma) * half_w - 0.5)
                .long()
                .clamp(0, featmap_size[1] - 1)
            )
            pos_top = (
                torch.ceil(gt_bboxes[:, 1] + (1 - self.sigma) * half_h - 0.5)
                .long()
                .clamp(0, featmap_size[0] - 1)
            )
            pos_down = (
                torch.floor(gt_bboxes[:, 1] + (1 + self.sigma) * half_h - 0.5)
                .long()
                .clamp(0, featmap_size[0] - 1)
            )
            for px1, py1, px2, py2, label, (gt_x1, gt_y1, gt_x2, gt_y2) in zip(
                ...
                bbox_targets[py1 : py2 + 1, px1 : px2 + 1, 0] = (
                    x[py1 : py2 + 1, px1 : px2 + 1] - gt_x1
                ) / base_len
                bbox_targets[py1 : py2 + 1, px1 : px2 + 1, 1] = (
                    y[py1 : py2 + 1, px1 : px2 + 1] - gt_y1
                ) / base_len
                bbox_targets[py1 : py2 + 1, px1 : px2 + 1, 2] = (
                    gt_x2 - x[py1 : py2 + 1, px1 : px2 + 1]
                ) / base_len
                bbox_targets[py1 : py2 + 1, px1 : px2 + 1, 3] = (
                    gt_y2 - y[py1 : py2 + 1, px1 : px2 + 1]
                ) / base_len
    ...
    def _bbox_decode(
        self, priors: Tensor, bbox_pred: Tensor, base_len: int, max_shape: int
    ) -> Tensor:
