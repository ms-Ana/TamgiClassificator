...
class DDODHead(AnchorHead):
    ...
    def _init_layers(self) -> None:
        ...
        self.cls_num_pos_samples_per_level = [
            0.0 for _ in range(len(self.prior_generator.strides))
        ]
    ...
    def loss_reg_by_feat_single(
        ...
        iou_pred = iou_pred.permute(0, 2, 3, 1).reshape(
            -1,
        )
        ...
        pos_inds = (
            ((labels >= 0) & (labels < bg_class_ind)).nonzero(as_tuple=False).squeeze(1)
        )
        ...
        if len(pos_inds) > 0:
            ...
            loss_bbox = self.loss_bbox(
                pos_decode_bbox_pred, pos_decode_bbox_targets, avg_factor=avg_factor
            )
    ...
    def loss_by_feat(
        ...
        return dict(
            loss_cls=cls_losses_cls, loss_bbox=reg_losses_bbox, loss_iou=reg_losses_iou
        )
    ...
    def process_predictions_and_anchors(
        ...
        mlvl_bbox_pred_list = [
            bbox_pred.permute(0, 2, 3, 1).reshape(
                num_imgs, -1, self.num_base_priors * 4
            )
    ...
    def get_cls_targets(
        ...
        label_weights_list = images_to_levels(
            all_label_weights, num_level_anchors_list[0]
        )
        bbox_targets_list = images_to_levels(
            all_bbox_targets, num_level_anchors_list[0]
        )
        bbox_weights_list = images_to_levels(
            all_bbox_weights, num_level_anchors_list[0]
        )
        return (
            anchors_list,
            labels_list,
