...
class GuidedAnchorHead(AnchorHead):
    ...
    def __init__(
        ...
        loss_shape: ConfigType = dict(type="BoundedIoULoss", beta=0.2, loss_weight=1.0),
        ...
        init_cfg: MultiConfig = dict(
            ...
            layer="Conv2d",
            std=0.01,
            override=dict(type="Normal", name="conv_loc", std=0.01, lbias_prob=0.01),
        ),
    ) -> None:
    ...
    def get_sampled_approxs(
        ...
        for img_id, img_meta in enumerate(batch_img_metas):
            ...
            multi_level_approx_flags = self.approx_anchor_generator.valid_flags(
    ...
    def ga_loc_targets(
        self, batch_gt_instances: InstanceList, featmap_sizes: List[Tuple[int, int]]
    ) -> tuple:
        ...
        for img_id in range(img_per_gpu):
            ...
            for gt_id in range(gt_bboxes.size(0)):
                ...
                if lvl < num_lvls - 1:
                    ...
                    all_ignore_map[u_lvl][
                        img_id, 0, ignore_y1 : ignore_y2 + 1, ignore_x1 : ignore_x2 + 1
                    ] = 1
        for lvl_id in range(num_lvls):
            # ignore negative regions w.r.t. ignore map
            all_loc_weights[lvl_id][
                (all_loc_weights[lvl_id] < 0) & (all_ignore_map[lvl_id] > 0)
            ] = 0
    ...
    def ga_shape_targets(
        ...
        return (bbox_anchors_list, bbox_gts_list, bbox_weights_list, avg_factor)
    ...
    def loss_by_feat(
        ...
        squares_list, guided_anchors_list, _ = self.get_anchors(
            featmap_sizes, shape_preds, loc_preds, batch_img_metas, device=device
        )

        # get shape targets
        shape_targets = self.ga_shape_targets(
            approxs_list,
            inside_flag_list,
            squares_list,
            batch_gt_instances,
        ...
        (bbox_anchors_list, bbox_gts_list, anchor_weights_list, ga_avg_factor) = (
            shape_targets
        )
        ...
        for i in range(len(loc_preds)):
            loss_loc = self.loss_loc_single(
                loc_preds[i], loc_targets[i], loc_weights[i], avg_factor=loc_avg_factor
            )
    ...
    def _predict_by_feat_single(
        ...
        if rescale:
            ...
            mlvl_bboxes /= mlvl_bboxes.new_tensor(img_meta["scale_factor"]).repeat(
                (1, 2)
            )
