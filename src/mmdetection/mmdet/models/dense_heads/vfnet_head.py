...
class VFNetHead(ATSSHead, FCOSHead):
    ...
    def __init__(
        ...
        loss_bbox: ConfigType = dict(type="GIoULoss", loss_weight=1.5),
        loss_bbox_refine: ConfigType = dict(type="GIoULoss", loss_weight=2.0),
    ...
    def forward_single(
        self, x: Tensor, scale: Scale, scale_refine: Scale, stride: int, reg_denom: int
    ) -> tuple:
        ...
        # trained with FP16
        dcn_offset = self.star_dcn_offset(bbox_pred, self.gradient_mul, stride).to(
            reg_feat.dtype
        )
    ...
    def star_dcn_offset(
        self, bbox_pred: Tensor, gradient_mul: float, stride: int
    ) -> Tensor:
    ...
    def loss_by_feat(
        ...
        if self.use_vfl:
            loss_cls = self.loss_cls(
                flatten_cls_scores, cls_iou_targets, avg_factor=num_pos_avg_per_gpu
            )
        else:
            loss_cls = self.loss_cls(
                flatten_cls_scores,
        ...
        return dict(
            loss_cls=loss_cls, loss_bbox=loss_bbox, loss_bbox_rf=loss_bbox_refine
        )
    ...
    def transform_bbox_targets(
        self, decoded_bboxes: List[Tensor], mlvl_points: List[Tensor], num_imgs: int
    ) -> List[Tensor]:
        """Transform bbox_targets (x1, y1, x2, y2) into (l, t, r, b) format.
