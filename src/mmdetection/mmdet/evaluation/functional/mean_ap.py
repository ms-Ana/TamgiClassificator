            ...
            for i in sort_inds:
                ...
                if not box_is_covered[i]:
                    if ioas_max[i] >= ioa_thr:
                        if not (
                            gt_ignore_inds[matched_gt] or gt_area_ignore[matched_gt]
                        ):
...
def eval_map(
    ...
    assert eval_mode in ["area", "11points"], (
        f'Unrecognized {eval_mode} mode, only "area" and "11points" ' "are supported"
    )
    if not use_legacy_coordinate:
        extra_length = 0.0
    else:
    ...
    for i in range(num_classes):
        ...
        if num_imgs > 1:
            ...
            tpfp = tpfp_fn(
                ...
                gt_bboxes_group_of=(
                    get_cls_group_ofs(annotations, i)[0] if use_group_of else None
                ),
