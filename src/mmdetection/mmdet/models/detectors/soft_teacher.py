...
class SoftTeacher(SemiBaseDetector):
    ...
    def loss_by_pseudo_instances(
        ...
        losses.update(
            **self.rcnn_cls_loss_by_pseudo_instances(
                x, rpn_results_list, batch_data_samples, batch_info
            )
        )
        losses.update(
            **self.rcnn_reg_loss_by_pseudo_instances(
    ...
    def get_pseudo_instances(
        ...
        for data_samples, reg_uncs in zip(batch_data_samples, reg_uncs_list):
            ...
            data_samples.gt_instances.bboxes = bbox_project(
                ...
                .inverse()
                .to(self.data_preprocessor.device),
    ...
    def rpn_loss_by_pseudo_instances(
    ...
    def rcnn_cls_loss_by_pseudo_instances(
        self,
        x: Tuple[Tensor],
        ...
        for bboxes, data_samples, teacher_matrix, teacher_img_shape in zip(
            selected_bboxes,
            batch_data_samples,
            batch_info["homography_matrix"],
            batch_info["img_shape"],
        ):
            ...
            projected_bboxes = bbox_project(
                bboxes, homography_matrix, teacher_img_shape
            )
        ...
        with torch.no_grad():
            ...
            bg_score = torch.cat([results.scores[:, -1] for results in results_list])
            # cls_reg_targets[0] is labels
            neg_inds = cls_reg_targets[0] == self.student.roi_head.bbox_head.num_classes
        ...
        losses = self.student.roi_head.bbox_head.loss(
            bbox_results["cls_score"], bbox_results["bbox_pred"], rois, *cls_reg_targets
        )
        # cls_reg_targets[1] is label_weights
        losses["loss_cls"] = (
            losses["loss_cls"]
            * len(cls_reg_targets[1])
            / max(sum(cls_reg_targets[1]), 1.0)
        )
        return losses
    ...
    def rcnn_reg_loss_by_pseudo_instances(
        ...
        roi_losses = self.student.roi_head.loss(x, rpn_results_list, reg_data_samples)
    ...
    def compute_uncertainty_with_aug(
        ...
        """
        auged_results_list = self.aug_box(
            batch_data_samples,
        ...
        reg_channel = max([results.bboxes.shape[-1] for results in results_list]) // 4
        bboxes = [
            (
                results.bboxes.reshape(
                    self.semi_train_cfg.jitter_times, -1, results.bboxes.shape[-1]
                )
                if results.bboxes.numel() > 0
                else results.bboxes.new_zeros(
                    self.semi_train_cfg.jitter_times, 0, 4 * reg_channel
                ).float()
            )
        ...
        box_unc = [
            (
                torch.mean(unc / wh[:, None, :].expand(-1, 2, 2).reshape(-1, 4), dim=-1)
                if wh.numel() > 0
                else unc
            )
    ...
    def aug_box(batch_data_samples, times, frac):
        ...
        def _aug_single(box):
            ...
            new_box = box.clone()[None, ...].expand(times, box.shape[0], -1) + offset
            return new_box
