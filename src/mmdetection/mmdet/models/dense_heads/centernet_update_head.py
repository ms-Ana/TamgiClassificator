...
def _transpose(tensor_list: List[Tensor], num_point_list: list) -> List[Tensor]:
    """This function is used to transpose image first tensors to level first
    ones."""
    for img_idx in range(len(tensor_list)):
        tensor_list[img_idx] = torch.split(tensor_list[img_idx], num_point_list, dim=0)
...
class CenterNetUpdateHead(AnchorFreeHead):
    ...
    def __init__(
        ...
        loss_cls: ConfigType = dict(
            type="GaussianFocalLoss", pos_weight=0.25, neg_weight=0.75, loss_weight=1.0
        ),
        loss_bbox: ConfigType = dict(type="GIoULoss", loss_weight=2.0),
    ...
    def loss_by_feat(
        ...
        cls_targets, bbox_targets = self.get_targets(
        ...
        pos_inds, cls_labels = self.add_cls_pos_inds(
            flatten_points, flatten_bbox_preds, featmap_sizes, batch_gt_instances
        )
        ...
        if pos_inds is None:
            ...
            num_pos_cls = bbox_preds[0].new_tensor(len(pos_inds), dtype=torch.float)
        ...
        # 5 calc reg loss
        pos_bbox_inds = torch.nonzero(bbox_targets.max(dim=1)[0] >= 0).squeeze(1)
    ...
    def _get_targets_single(
        ...
        if num_gts == 0:
            return gt_labels.new_full(
                (num_points, self.num_classes), self.num_classes
        ...
        centers_discret = ((centers / strides).int() * strides).float() + strides / 2
        ...
        bbox_target_mask = (
            inside_gt_bbox_mask & inside_gt_center3x3_mask & inside_fpn_level_mask
        )
        ...
        bbox_target = bbox_target[
            range(len(bbox_target)), min_inds
        ]  # M x N x 4 --> M x 4
    ...
    def add_cls_pos_inds(
        ...
        num_gts, total_level, K = (
            cls_labels.shape[0],
            len(self.strides),
            center3x3_masks.shape[-1],
        )
        ...
        center3x3_bbox_targets_expand = center3x3_bbox_targets.view(-1, 4).clamp(min=0)
        ...
        loss_thr = torch.kthvalue(center3x3_bbox_loss, self.more_pos_topk, dim=1)[0]
        ...
        cls_labels = cls_labels.view(num_gts, 1).expand(num_gts, total_level * K)[
            new_pos
        ]
        return pos_inds, cls_labels

    def _create_heatmaps_from_dist(
        self, weighted_dist: Tensor, cls_labels: Tensor
    ) -> Tensor:
        """Generate heatmaps of classification branch based on weighted
        distance map."""
        heatmaps = weighted_dist.new_zeros((weighted_dist.shape[0], self.num_classes))
    ...
    def _get_center3x3_region_index_targets(
        self, bacth_gt_instances: InstanceList, shapes_per_level: Tensor
    ) -> tuple:
        ...
        K = 9
        dx = shapes_per_level.new_tensor([-1, 0, 1, -1, 0, 1, -1, 0, 1]).view(1, 1, K)
        dy = shapes_per_level.new_tensor([-1, -1, -1, 0, 0, 0, 1, 1, 1]).view(1, 1, K)
        ...
        start_coord_pre_level = shapes_per_level.new_tensor(start_coord_pre_level).view(
            1, total_levels, 1
        )
        ...
        for im_i in range(batch):
            ...
            expanded_strides = strides[None, :, None].expand(num_gts, total_levels, 2)
            ...
            centers_discret = centers_inds * expanded_strides + expanded_strides // 2

            bbox_target = bbox2distance(centers_discret, gt_bboxes)  # M x N x 4

            # calc inside_fpn_level_mask
            bbox_target_wh = bbox_target[..., :2] + bbox_target[..., 2:]
            crit = (bbox_target_wh**2).sum(dim=2) ** 0.5 / 2
            inside_fpn_level_mask = (crit >= expanded_regress_ranges[..., 0]) & (
            ...
            center3x3_bbox_target = (
                bbox_target[..., None, :].expand(num_gts, total_levels, K, 4).clone()
            )
        ...
        if len(inside_fpn_level_masks) > 0:
            ...
            center3x3_inds = shapes_per_level.new_zeros((0, total_levels, K)).long()
            center3x3_bbox_targets = shapes_per_level.new_zeros(
                (0, total_levels, K, 4)
            ).float()
            center3x3_masks = shapes_per_level.new_zeros((0, total_levels, K)).bool()
        return (
            cls_labels,
            inside_fpn_level_masks,
            center3x3_inds,
