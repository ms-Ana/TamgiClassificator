...
class RTMDetInsHead(RTMDetHead):
    ...
    def _predict_by_feat_single(
        ...
        for level_idx, (
            ...
            results = filter_scores_and_topk(
                ...
                score_thr,
                nms_pre,
                dict(bbox_pred=bbox_pred, priors=priors, kernel_pred=kernel_pred),
            )
    ...
    def _mask_predict_by_feat_single(
        self, mask_feat: Tensor, kernels: Tensor, priors: Tensor
    ) -> Tensor:
        ...
        if num_inst < 1:
            return torch.empty(
                size=(num_inst, h, w), dtype=mask_feat.dtype, device=mask_feat.device
            )
        if len(mask_feat.shape) < 4:
            mask_feat.unsqueeze(0)

        coord = self.prior_generator.single_level_grid_priors(
            (h, w), level_idx=0, device=mask_feat.device
        ).reshape(1, -1, 2)
        num_inst = priors.shape[0]
    ...
    def loss_mask_by_feat(
        ...
        loss_mask = self.loss_mask(
            batch_pos_mask_logits, pos_gt_masks, weight=None, avg_factor=num_pos
        )
    ...
    def loss_by_feat(
        ...
        loss_mask = self.loss_mask_by_feat(
            mask_feat, flatten_kernels, sampling_results_list, batch_gt_instances
        )
...
class RTMDetInsSepBNHead(RTMDetInsHead):
    ...
    def init_weights(self) -> None:
        ...
        for rtm_cls, rtm_reg, rtm_kernel in zip(
            self.rtm_cls, self.rtm_reg, self.rtm_kernel
        ):
