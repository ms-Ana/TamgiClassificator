...
class IoUBalancedNegSampler(RandomSampler):
    ...
    def __init__(
        self, num, pos_fraction, floor_thr=-1, floor_fraction=0, num_bins=3, **kwargs
    ):
    ...
    def sample_via_interval(self, max_overlaps, full_set, num_expected):
        ...
        for i in range(self.num_bins):
            ...
            tmp_set = set(
                np.where(
                    np.logical_and(max_overlaps >= start_iou, max_overlaps < end_iou)
                )[0]
            )
    ...
    def _sample_neg(self, assign_result, num_expected, **kwargs):
        ...
        if len(neg_inds) <= num_expected:
            ...
            if self.floor_thr > 0:
                floor_set = set(
                    np.where(
                        np.logical_and(max_overlaps >= 0, max_overlaps < self.floor_thr)
                    )[0]
                )
            ...
            if len(iou_sampling_neg_inds) > num_expected_iou_sampling:
                ...
                iou_sampled_inds = np.array(iou_sampling_neg_inds, dtype=np.int64)
